      ...
      <div class="resumen-card card">
        ...
        <div class="kcal-section">
          ...
          <div class="progress-track">
            <div
              class="progress-fill"
              :style="{ width: pctKcal + '%', background: colorKcal }"
            ></div>
          </div>
        </div>
        ...
        <div class="macros-row">
          <div class="macro-item" v-for="m in macros" :key="m.key">
            <div class="macro-circle" :style="{ '--pct': m.pct + '%', '--color': m.color }">
        ...
        <!-- Métricas extra según padecimientos -->
        ...
        <div class="semaforo-dia">
          ...
          <div class="sd-bar">
            <div class="sd-seg verde"  :style="{ flex: totales.verde  || 0 }" :title="`${totales.verde} verde`"></div>
            <div class="sd-seg amarillo" :style="{ flex: totales.amarillo || 0 }" :title="`${totales.amarillo} amarillo`"></div>
            <div class="sd-seg rojo"  :style="{ flex: totales.rojo   || 0 }" :title="`${totales.rojo} rojo`"></div>
          </div>
          <div class="sd-counts">
          ...
          >
            ...
            <div class="entrada-info">
              ...
              <span class="entrada-meta">
                <template v-if="entrada.origen === 'alimento'">
                  {{ entrada.cantidad_g }}g
                </template>
                <template v-else>
                  {{ entrada.receta_porciones }} porción(es)
                </template>
              </span>
            </div>
      ...
      <div v-if="modal.open" class="modal-overlay" @click.self="cerrarModal">
        <div class="modal-card">
          ...
          <div class="modal-tabs">
            <button
              class="modal-tab" :class="{ active: modal.origen === 'alimento' }"
              @click="modal.origen = 'alimento'; modal.seleccionado = null"
            >🥗 Alimento</button>
            <button
              class="modal-tab" :class="{ active: modal.origen === 'receta' }"
              @click="modal.origen = 'receta'; modal.seleccionado = null"
            >🍽️ Receta</button>
          </div>
          ...
          <template v-if="modal.origen === 'alimento'">
            ...
            <div v-if="modal.resultados.length" class="resultados-list">
              <button
                v-for="a in modal.resultados" :key="a.nombre"
...
function irDia(delta) {
  ...
  fechaActual.value = d.toISOString().slice(0, 10)
}
  ...
  for (const meal of mealTypes) {
    m[meal.key] = Math.round(
      (entradasPorTipo.value[meal.key] || []).reduce((acc, e) => {
        return acc + parseFloat(e.origen === 'receta' ? e.receta_kcal || 0 : e.alimento_kcal || 0)
      }, 0)
    )
  }
  return m
...
].map(m => ({ ...m, pct: computed(() => Math.min(100, Math.round((totales.value[m.key] / m.max) * 100))) }))

// Métricas extra solo para padecimientos activos
...
const modal = reactive({
  ...
  seleccionado: null, cantidad: 100, porciones: 1,
})
...
async function buscarRecetas() {
  const q = modal.busqueda.toLowerCase().trim()
  if (q.length < 2) { modal.resultadosRecetas = []; return }
  const { data } = await supabase
    .from('recetas')
    .select('id, nombre, kcal_total, porciones')
    .eq('activa', true)
    .ilike('nombre', `%${q}%`)
    .limit(8)
  modal.resultadosRecetas = data || []
}
...
const kcalPreview = computed(() => {
  ...
  const base = parseFloat(modal.seleccionado.energia_kcal) || 0
  const baseG = parseFloat(modal.seleccionado.peso_neto_g) || 100
  return Math.round((base / baseG) * (modal.cantidad || 0))
})
...
async function guardarEntrada() {
  ...
  try {
    ...
    if (modal.origen === 'alimento') {
      ...
      const baseG = parseFloat(a.peso_neto_g) || 100
      const factor = (modal.cantidad || 100) / baseG
      const color  = clasificarAlimento(a, condiciones.value)

      payload = {
        ...payload,
        alimento_nombre: a.nombre,
        alimento_kcal:   Math.round((parseFloat(a.energia_kcal)         || 0) * factor * 10) / 10,
        alimento_prot:   Math.round((parseFloat(a.proteina_g)           || 0) * factor * 10) / 10,
        alimento_carbs:  Math.round((parseFloat(a.hidratos_carbono_g)   || 0) * factor * 10) / 10,
        alimento_grasas: Math.round((parseFloat(a.lipidos_g)            || 0) * factor * 10) / 10,
        alimento_fibra:  Math.round((parseFloat(a.fibra_g)              || 0) * factor * 10) / 10,
        alimento_sodio:  Math.round((parseFloat(a.sodio_mg)             || 0) * factor * 10) / 10,
        alimento_azucar: Math.round((parseFloat(a.azucar_g)             || 0) * factor * 10) / 10,
      ...
      const r = modal.seleccionado
      payload = {
        ...payload,
        receta_id:       r.id,
        receta_nombre:   r.nombre,
        receta_kcal:     Math.round((parseFloat(r.kcal_total) || 0) * (modal.porciones || 1) / (r.porciones || 1)),
        receta_porciones: modal.porciones,
      }
    }
...
.fecha-badge {
  ...
  background: var(--gray-100); padding: 4px 12px; border-radius: 99px;
...
.progress-track {
  width: 100%; height: 8px; background: var(--gray-100); border-radius: 99px; overflow: hidden;
}
...
.del-btn {
  ...
  font-size: 12px; padding: 4px; border-radius: 4px; transition: all .2s;
}
...
/* ── Empty card ── */
.empty-card {
  display: flex; flex-direction: column; align-items: center; gap: 12px; text-align: center; padding: 40px 24px;
}
...
.preview-kcal {
  display: flex; align-items: baseline; gap: 4px; padding-bottom: 14px;
  white-space: nowrap;
}
